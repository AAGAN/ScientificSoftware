...
@pytest.mark.parametrize(
    ("Temperature", "SpecificHeat"), ((77, 192), (100, 252), (150, 323))
)
def test_specific_heat_capacity_interpolation(SSP_Instance, Temperature, SpecificHeat):
    assert SSP_Instance._ScientificSoftwareProblem__SpecificHeatCapacityFunc(
        Temperature
    ) == pytest.approx(
        SpecificHeat
    )  # testing the correctness of interpolation for specific heat capacity of copper


@pytest.mark.parametrize(
    ("Temperature", "Resistivity"), ((77, 0.21e-8), (100, 0.34e-8), (150, 0.7e-8))
)
def test_cu_resistivity_interpolation(SSP_Instance, Temperature, Resistivity):
    assert (
        SSP_Instance._ScientificSoftwareProblem__CuResistivityElectricalResistivityFunc(
            Temperature
        )
        == pytest.approx(Resistivity)
    )
...
def test_dT_dt(SSP_Instance):
    ...
    assert (
        33.8 < SSP_Instance._ScientificSoftwareProblem__dT_dt(0, T) < 33.9
    )  # testing the conservation of energy's ODE quation
...
def test_magnetic_field_magnitude(SSP_Instance):
    ...
    assert (
        8.815
        < SSP_Instance._ScientificSoftwareProblem__MagneticFieldMagnitude()
        < 8.825
    )  # testing the equation for Ampere's law


def test_solve_RK4(
    SSP_Instance,
):  # Test that the calculation of Pulse Duration is done and values are > 0 when solving with RK4 method
...
def test_solve_ivp(
    SSP_Instance,
):  # Test that the calculation of Pulse Duration is done and values are > 0 when solving with scipy.integrate.solve_ivp method
