...
class ScientificSoftwareProblem:
    ...
    def __init__(
        ...
        self.current_densities_to_plot_A_per_m2 = self.TFCoilsAlcatorCmodData[
            "current_densities_to_plot_A_per_m2"
        ]
        ...
        self.number_turns_per_tf_coil = self.TFCoilsAlcatorCmodData[
            "number_turns_per_tf_coil"
        ]
        self.cross_section_area_turn_m2 = self.TFCoilsAlcatorCmodData[
            "cross_section_area_turn_m2"
        ]
        self.coil_temperature_initial_K = self.TFCoilsAlcatorCmodData[
            "coil_temperatures_K"
        ]["initial"]
        self.coil_temperature_final_K = self.TFCoilsAlcatorCmodData[
            "coil_temperatures_K"
        ]["final"]
    ...
    def __SpecificHeatCapacityFunc(self, T, n=1):
        ...
        if n == 1:
            interpolated_value = np.interp(
                T, self.TemperatureC, self.SpecificHeatCapacity
            )
        else:
    ...
    def __CuResistivityElectricalResistivityFunc(self, T, n=1):
        ...
        if n == 1:
            interpolated_value = np.interp(
                T, self.TemperatureRho_e, self.CuElectricalResistivity
            )
        else:
            coefficients = np.polyfit(
                self.TemperatureRho_e, self.CuElectricalResistivity, n
            )
            interpolated_value = np.polyval(coefficients, T)
    ...
    def __MagneticFieldMagnitude(self):
        ...
        B0 = (
            self.mu_0
            * self.number_tf_coils
            * self.number_turns_per_tf_coil
            * self.cross_section_area_turn_m2
            * self.jiterator
            / 2.0
            / np.pi
            / self.major_radius_m
    ...
    def __plot_results(self):
        ...
        ax1.plot(
            self.current_densities_to_plot_A_per_m2,
            self.PulseDuration,
            marker="x",
    ...
    def __print_results(self):
        ...
        for i in range(len(self.current_densities_to_plot_A_per_m2)):
            row = "{:18.2e} {:28.4e} {:22.4e}".format(
                self.current_densities_to_plot_A_per_m2[i],
                self.MagneticField[i],
